...
TransactionBuilder.prototype._setInputs = function(txobj) {
  ...
  var l = ins.length;
  var valueInSat = new BN(0);  // Initialize with BN instead of bignum

  txobj.ins = [];
  for (var i = 0; i < l; i++) {
    // Assuming util.parseValue returns a string that can be converted to BN
    valueInSat = valueInSat.add(new BN(util.parseValue(ins[i].amount)));
  ...
  this.valueInSat = valueInSat;  // This will now hold a BN instance
  return this;
};
...
TransactionBuilder.prototype._setRemainder = function(txobj, remainderIndex) {
  ...
  // Ensure that valueInSat and valueOutSat are BN instances and use BN operations
  var remainderSat = this.valueInSat.sub(this.valueOutSat).sub(this.feeSat);
  var l = txobj.outs.length;
  this.remainderSat = new BN(0);  // Initialize as BN
  ...
  if (remainderSat.cmp(new BN(0)) > 0) {  // Use BN's cmp method to compare
    var remainderOut = this.remainderOut || this.selectedUtxos[0];
    
    // Ensure bigIntToValue handles BN objects (may need adjustment if not already compatible)
    var value = remainderSat.toString();
    ...
    this.remainderSat = remainderSat;  // Keep as BN instance
  }
...
TransactionBuilder.prototype.setOutputs = function(outs) {
  ...
  var valueOutSat = new BN(0);  // Initialize as BN
  var txobj = {};
  txobj.version = 1;
  ...
  var l = outs.length;
  for (var i = 0; i < l; i++) {
    // Convert the amount from string or from the parsed value to BN
    var amountSat = outs[i].amountSat || outs[i].amountSatStr ? new BN(outs[i].amountSatStr) : new BN(util.parseValue(outs[i].amount));

    // Ensure bigIntToValue handles BN objects (this might need adjustment if not already compatible)
    var value = amountSat.toString(); //util.bigIntToValue(amountSat);
  ...
  this.valueOutSat = valueOutSat;  // Now a BN instance
